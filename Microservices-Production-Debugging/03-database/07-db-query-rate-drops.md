...
Business success collapses while the database dashboard misleadingly shows fewer queries and normal CPU.
...
At 10:31, eleven minutes after validation release 2026.09.13, `POST /checkout/validate` degraded.

* Application request rate: 2,000/min unchanged
* Normal endpoint p95: 300 ms
* Current endpoint p95: 5.1 s
* Business/error signal: 21% business failures
* CPU: MySQL 26%, application 29%
* HikariCP: active 40/40, idle 0, pending 186, acquire p99 4.9 s
* Database evidence: query rate fell from 1,700/s to 590/s; executed query p95 stayed 34 ms
* Heap: 56%; maximum GC pause: 24 ms
* Service-to-service transport: normal

Representative SQL shape: `SELECT status,available FROM inventory WHERE sku=?`
...
* WHAT RESULT AM I LOOKING FOR: Requests spend 4.9 s before their first DB span or end without one; executed DB spans remain about 34 ms.
...
* Expected result: rate near 2,000/min unchanged and p95 near 300 ms
* Bad result: p95 5.1 s and 21% business failures
...
* Expected result: Requests spend 4.9 s before their first DB span or end without one; executed DB spans remain about 34 ms.
...
* Expected result: query rate fell from 1,700/s to 590/s; executed query p95 stayed 34 ms
...
* Example command/query/tool: `EXPLAIN FORMAT=TREE SELECT status,available FROM inventory WHERE sku='SKU-441'`
...
* Query rate is throughput, not health. A fall with unchanged demand can mean upstream admission is blocked.
* Walk acquisition, DNS/TCP/TLS/login, transaction, lock wait, execution, transfer/mapping, and commit as separate phases.
* Checkout/return divergence establishes lost ownership more strongly than one Hikari leak-warning log.
* Retries increase pending work but cannot create connection capacity, so they amplify the outage.
...
Observed evidence: Requests spend 4.9 s before their first DB span or end without one; executed DB spans remain about 34 ms.
...
Gateway                         19 ms
  | parent spanId=gw-97
  v
POST /checkout/validate service      5.1 s
  | spanId=validate-72
  +-- hikari.acquire             4,903 ms ERROR
...
```text
2026-09-13T10:31:22.481Z WARN service=checkout-validation-service instance=validation-2b6e endpoint="/checkout/validate"
traceId=rate-d92f spanId=validate-72 requestId=validate-903 version=2026.09.13
pool=HikariPool-validation active=40 idle=0 pending=186 acquireMs=4903
error=SQLTransientConnectionException timeoutMs=5000 connectionReturned=false
2026-09-13T10:31:22.486Z INFO service=checkout-validation-service instance=validation-2b6e
traceId=rate-ok-18 spanId=mysql-93 statement=inventory.availability
queryDigest=inventory-availability-by-sku executeMs=34 rowsReturned=1
```
...
EXPLAIN FORMAT=TREE SELECT status,available FROM inventory WHERE sku='SKU-441';
...
A deployment leaked JDBC connections on the validation-error path; fewer requests crossed pool acquisition, while retries amplified the application queue.

Causal chain:

```text
validation-error path loses connection ownership
  -> checkout count continues while return count stalls
  -> active reaches 40/40, idle reaches 0, and pending reaches 186
  -> requests spend 4.9 s acquiring and never create a SQL span
  -> query rate falls from 1,700/s to 590/s despite constant demand
  -> MySQL CPU falls to 26% and executed-query p95 stays 34 ms
  -> retries add pending work and business failures reach 21%
```
...
* Rollback and drain only affected instances, suppress retry amplification, and preserve pool lifecycle evidence.
...
* Remove manual connection ownership, guarantee cleanup with Spring transactions or try-with-resources, and add checkout/return, pending, and acquisition tests and alerts.
...
|---|---:|---:|---|
| Endpoint p95 | 5.1 s | near 300 ms | customer latency recovered |
| Business/error signal | 21% business failures | below 1% and baseline | business success recovered |
...
| DB query rate | 590/s | returns toward 1,700/s | requests reach MySQL again |
| Inventory SELECT p95 | 34 ms | at or below 34 ms | SQL never required tuning |
| MySQL CPU | 26% during blockage | rises only with restored throughput | low CPU was starvation, not health |
...
### 1. Why is lower query rate not good news?

Demand is unchanged, but pool wait prevents work reaching MySQL; throughput fell because the path is blocked.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 2. Which charts should be overlaid?

App rate, business success, pool pending/acquire, DB query rate, DB latency, and checkout/return delta.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 3. Could login failure look similar?

Yes, but new-connection creation shows DNS/TCP/TLS/auth errors rather than checked-out connections never returning.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 4. How do traces distinguish it?

Acquire dominates and execute spans are absent, while the few executed statements remain fast.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 5. Why drain instances?

It stops new traffic and lets in-flight work finish more safely than restarting the fleet.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 6. What prevents recurrence?

Structured ownership, failure-path tests, leak diagnostics, and alerts on pending plus lifecycle divergence.
