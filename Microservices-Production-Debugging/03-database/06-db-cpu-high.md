...
MySQL CPU saturates during catalog search while application CPU remains moderate.

The affected operation is `GET /catalog/search`.
...
At 16:15, seven minutes after enabling optional catalog filters for all tenants, `GET /catalog/search` degraded.

* Application request rate: 3,400/min
* Normal endpoint p95: 280 ms
* Current endpoint p95: 3.9 s
* Business/error signal: 9% timeouts
* CPU: MySQL 94%, application 43%
* HikariCP: active 39/40, idle 1, pending 73, acquire p99 1.1 s
* Database evidence: 1.8 billion rows examined/min; on-disk temp tables rose 8x
* Heap: 58%; maximum GC pause: 27 ms
* Service-to-service transport: normal

Representative SQL shape: `SELECT id,sku,name,price FROM product WHERE tenant_id=? AND status=? AND name LIKE ? ORDER BY COALESCE(updated_at,created_at) DESC LIMIT 50`
...
* WHAT RESULT AM I LOOKING FOR: Search SQL spans expand from 120 ms to 3.6 s with server CPU and disk temporary tables.
...
* What I check: RED and business metrics for GET /catalog/search
...
* Expected result: rate near 3,400/min and p95 near 280 ms
* Bad result: p95 3.9 s and 9% timeouts
...
* Expected result: Search SQL spans expand from 120 ms to 3.6 s with server CPU and disk temporary tables.
...
* Expected result: 1.8 billion rows examined/min; on-disk temp tables rose 8x
...
* Example command/query/tool: `EXPLAIN FORMAT=TREE SELECT id,sku,name,price FROM product WHERE tenant_id=42 AND status='ACTIVE' AND name LIKE '%fiber%' ORDER BY COALESCE(updated_at,created_at) DESC LIMIT 50`
...
* High CPU proves work is occurring but not whether it is one query, aggregate concurrency, scans, joins, sorts, or background work.
* Access type ALL, large rows examined, filesort, and disk temporary tables explain cost only when linked to this digest and traffic.
* Estimated/actual mismatch points toward statistics, skew, correlated predicates, or parameter-sensitive plans.
* Protect the service with bounded contracts while the plan and schema change receives evidence-based review.
...
Observed evidence: Search SQL spans expand from 120 ms to 3.6 s with server CPU and disk temporary tables.
...
Gateway                         21 ms
  | parent spanId=gw-84
  v
GET /catalog/search service      3.9 s
  | spanId=catalog-52
  +-- hikari.acquire               184 ms
  +-- transaction.begin             2 ms
  +-- mysql.catalog-search       3,611 ms
  +-- jdbc.fetch/map                21 ms
  +-- transaction.commit             3 ms
```
...
```text
2026-09-13T16:15:31.700Z WARN service=catalog-service instance=catalog-9c3f endpoint="/catalog/search"
traceId=cpu-c73a spanId=catalog-52 requestId=catalog-441 version=2026.09.13
pool=HikariPool-catalog active=39 idle=1 pending=73 acquireMs=184 acquireP99Ms=1100 usageMs=3640
2026-09-13T16:15:35.311Z WARN service=catalog-service instance=catalog-9c3f
traceId=cpu-c73a spanId=mysql-88 statement=catalog.optional-filter-search
queryDigest=catalog-search executeMs=3611 rowsExamined=840000 rowsSent=50 accessType=ALL filesort=true tmpDiskTables=1
```
...
WHERE SCHEMA_NAME='catalog_db'
ORDER BY SUM_TIMER_WAIT DESC LIMIT 20;
EXPLAIN FORMAT=TREE SELECT id,sku,name,price FROM product
WHERE tenant_id=42 AND status='ACTIVE' AND name LIKE '%fiber%'
...
A new optional-filter query used low-selectivity predicates and expression sorting; skew selected a scan plan that sorted common-value results to disk.

Causal chain:

```text
optional-filter rollout permits broad common-value searches
  -> skew and expression ORDER BY select a scan plus filesort
  -> each request examines about 840,000 rows for 50 results
  -> disk temp-table work rises 8x and MySQL CPU reaches 94%
  -> execution reaches 3.6 s and connections remain busy
  -> active reaches 39, pending reaches 73, and acquire p99 reaches 1.1 s
  -> catalog timeout rate reaches 9%
```
...
* Disable the broadest search mode, enforce bounded filters, and rate-limit the expensive path through approved controls.
...
* Define supported query shapes, review a selective composite index and sortable column, refresh statistics safely, and move analytical search away from OLTP.
...
|---|---:|---:|---|
| Endpoint p95 | 3.9 s | near 280 ms | customer latency recovered |
| Business/error signal | 9% timeouts | below 1% and baseline | business success recovered |
| MySQL CPU | 94% | below 60% at the same rate | expensive server work fell |
| Rows examined/min | 1.8 billion | below 150 million | scans are bounded |
| Disk temp tables | 8x baseline | baseline | spill amplification removed |
| Catalog SELECT p95 | 3.6 s | below 220 ms | execution recovered |
| Hikari pending | 73 | 0 | secondary pool queue cleared |
| Acquire p99 | 1.1 s | below 20 ms | admission recovered after SQL shortened |
| Active/idle | 39/1 | below 28 and above 12 idle | connections are no longer pinned by scans |
...
For high mysql cpu, I first confirm business impact and scope by endpoint, instance, version, tenant, and parameter. Then I use a trace to separate Hikari acquisition, physical connect/login, transaction begin, lock wait, SQL execution, result mapping, and commit. I correlate traceId with logs and overlay app rate, pool active/idle/pending/acquire, DB query rate, statement digests, rows examined, locks, CPU, and I/O. Here, A new optional-filter query used low-selectivity predicates and expression sorting; skew selected a scan plan that sorted common-value results to disk. I use the smallest reversible approved mitigation, implement the permanent fix, and verify business success plus causal metrics at peak load.
...
### 1. Does high DB CPU prove one bad query?

No. One digest, many medium calls, concurrency, and background work are separate hypotheses.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 2. Which metrics connect CPU to SQL?

Digest execution count, total time, rows examined, temporary tables, and matching trace rate.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 3. What is data skew?

Values are unevenly distributed, so a plan good for rare values can fail for common values.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 4. What does filesort mean?

MySQL performs an extra sort; it can be in memory or spill and is not automatically wrong.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 5. Why cap results?

It bounds scan, transfer, mapping, serialization, and connection hold time.

I would still require same-window trace, metric, and server evidence before declaring root cause.

### 6. How do you validate an index?

Compare plans and production-like read/write tests, then deploy with approval, monitoring, and rollback.
